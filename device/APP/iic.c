...
* Attention		 : None
*******************************************************************************/           
FunctionalState I2C_WriteByte(uint8_t* pBuffer,uint16_t length, uint16_t WriteAddress, uint8_t DeviceAddress)
{		
	DeviceAddress<<=1;
    if(!I2C_Start())return DISABLE;
    I2C_SendByte(DeviceAddress & 0xFFFE); /*设置高起始地址+器件地址 */
...
/*********************************************************************************************************
      END FILE
*********************************************************************************************************/
FunctionalState I2C_WriteByteWithoutWA(uint8_t* pBuffer,uint16_t length, uint8_t DeviceAddress)
{		
	DeviceAddress<<=1;
    if(!I2C_Start())return DISABLE;
    I2C_SendByte(DeviceAddress & 0xFFFE); /*设置高起始地址+器件地址 */
...
FunctionalState I2C_ReadByteWithoutRA(uint8_t* pBuffer,   uint16_t length,  uint8_t DeviceAddress)
{		
	DeviceAddress<<=1;
    if(!I2C_Start())return DISABLE;
    I2C_SendByte( DeviceAddress | 0x0001);
